...
import Faron.sorm.bean.ColumnInfo;
import Faron.sorm.bean.TableInfo;
import Faron.sorm.utils.JDBCUtils;
import Faron.sorm.utils.ReflectUtils;
...
public Object executeQueryTemplate(String sql,Object[] params,Class clazz,CallBack back){
	...
	public int executeDML(String sql,Object[] params){
		Connection conn = DBManager.getConn();  //从连接池中获取对象
		int count = 0; 
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement(sql);
	...
	public void insert(Object obj){
		...
		//obj-->表中。             insert into 表名  (id,uname,pwd) values (?,?,?)     这种获取方式是在 数据类中的字段和 数据库中字段相同的情况下，如果不相同，则需要进行转换
		Class c = obj.getClass();
		List<Object> params = new ArrayList<Object>();   //存储sql的参数对象
		TableInfo tableInfo = TableContext.poClassTableMap.get(c);  //获取表结构信息
		StringBuilder sql  = new StringBuilder("insert into "+tableInfo.getTname()+" (");  //通过从表结构信息中获取的字段来 拼接sql语句
		int countNotNullField = 0;   //计算不为null的属性值
		Field[] fs = c.getDeclaredFields();        //传入了一个对象，通过这个和表对应的对象 获取对象中的所有属性
		for(Field f:fs){
			/*K*/String fieldName = f.getName();  //Returns the name of the field represented by this {@code Field} object. 返回的是一个属性的名称
			/*V*/Object fieldValue = ReflectUtils.invokeGet(fieldName, obj);   //调用obj对象对应属性fieldName的get方法  返回的是一个属性的值

			if(fieldValue!=null){  //当通过get方法 得到属性值不为空的时候
				countNotNullField++;    //通过这个值计算需要写几个占位符
				sql.append(fieldName+",");   //将这个属性名拼接到sql语句中   (id,uname,pwd)
				params.add(fieldValue);      //加入到参数数组中，executeDML(sql.toString(), params.toArray())的时候传入
			}
		}
