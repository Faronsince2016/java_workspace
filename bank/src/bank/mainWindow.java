...
class AddResourceWindow extends JFrame implements ActionListener{
    ...
    public AddResourceWindow(){
        ...
        for(int i=0; i<n; i++, y=y+40){
            ...
            a = (char)(a+1);
            AvailableTextField[i] = new JTextField(4); //4列
            AvailableTextField[i].setHorizontalAlignment(JTextField.RIGHT);//内容右对齐
...
class StartProcess extends JFrame implements ActionListener{
    ...
    public StartProcess(){
         ...
         *比如说 contaninername.setLayout(new GridLayout(1,3)) 
         *意思就是把contaninername这个组件设置成1行3列的网格布局
         * */
...
//      System.out.println();
    ...
    public boolean bankerAlgorithm(){
        ...
        if(isSafety()==false){ //分配失败时调用，恢复系统原状           
            for(int j=0; j<n; j++){
            	 //重新调整系统资源数  
    ...
    int P_id(String PName){
        for(int i=0; i<m; i++){
            if(processName[i].compareToIgnoreCase(PName) == 0) 
            return i;
        }
        return -1;
    }
